...
export const BuyerSalesOrderCreateScreen = () => {
  ...
  const loadItems = async () => {
    ...
    setLoading(true);
    try {
      const itemsRes = await getInventoryItems(token, { page: 1, limit: 200 });
  ...
  const handleCreate = async () => {
    if (!token) return;
    if (!user?.customerId) {
      Alert.alert('Validation', 'Your account is missing customer mapping.');
      return;
    }
    if (!lineItems.length) {
