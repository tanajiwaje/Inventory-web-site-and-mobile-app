...
export const AdminSalesOrdersScreen = () => {
  const { token } = useAuth();
  ...
  const handleApprove = async (order: SalesOrder) => {
    if (!token) return;
    try {
      await updateSalesOrder(token, order._id, { status: 'approved' });
      await loadOrders();
      showToast('Sales order approved.');
      addNotification('Sales approved', `Order for ${order.customer?.name || 'customer'} was approved.`);
    } catch (error) {
      Alert.alert('Error', String(error));
    }
  };
  ...
  const handleReceive = async (order: SalesOrder) => {
    ...
    try {
      ...
      await loadOrders();
      showToast('Sales order marked as received.');
      addNotification('Sales received', `Order for ${order.customer?.name || 'customer'} moved to received.`);
    } catch (error) {
      Alert.alert('Error', String(error));
    }
  };
  ...
  return (
    <Screen>
      <View style={styles.container}>
        ...
        <PrimaryButton label="New Sales Order" onPress={() => navigation.navigate('AdminSalesOrderCreate')} style={styles.createBtn} />
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <EmptyState
                ...
                subtitle="Try a different filter or create a new sales order."
                ...
                onAction={
                  search.trim() || statusFilter !== 'all'
                    ...
                    : () => navigation.navigate('AdminSalesOrderCreate')
                }
              />
            }
            renderItem={({ item }) => {
              ...
              return (
                <View style={styles.card}>
                  ...
                  {item.status === 'requested' ? <PrimaryButton label="Approve" onPress={() => handleApprove(item)} /> : null}
