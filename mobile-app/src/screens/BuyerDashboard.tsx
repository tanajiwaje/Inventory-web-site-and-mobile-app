...
export const BuyerDashboard = () => {
  ...
  useEffect(() => {
    const load = async () => {
      if (!token) return;
      const res = await getSalesOrders(token, { page: 1, limit: 200 });
    ...
    <Screen>
      ...
      <View style={styles.grid}>
        ...
        <Card>
          ...
          <Text style={styles.cardValue}>{statusCounts.received ?? 0}</Text>
        </Card>
        <TouchableOpacity onPress={() => navigation.navigate('BuyerSalesOrders')}>
          <Card>
            <Text style={styles.cardLabel}>Sales Orders</Text>
            <Text style={styles.cardValue}>Create and track orders</Text>
          </Card>
        </TouchableOpacity>
